...
class PublicSslCommerzPaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
            ...
            if(Session::has('payment_type')){
                ...
                elseif (Session::get('payment_type') == 'subscription') {
                    
                    $post_data = array();
                    $post_data['total_amount'] = Session::get('payment_data')['amount']; # You cant not pay less than 10
                    $post_data['currency'] = "BDT";
                    $post_data['tran_id'] = substr(md5($request->session()->get('payment_option')), 0, 10);
    ...
    public function success(Request $request)
    {
        ...
        if(isset($_SESSION['payment_values']['payment_type'])){
            ...
            elseif ($_SESSION['payment_values']['payment_type'] == 'subscription') {
                $subscription = new SubscriptionController;
                return $subscription->payment_done($_SESSION['payment_values']['payment_data'], $payment);
            }
        }
    }
