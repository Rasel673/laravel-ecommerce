...
class HomeController extends Controller
{
    ...
    public function show_product_upload_form(Request $request)
    {
        ...
        if(!empty($package))
        {
            if(\Carbon\Carbon::now()->format('Y-m-d') > $user->seller->date){
                flash(__("Your package were expired"))->success();
                return redirect()->back(); 
            }
            if (@$package->allowed_products > 0) {        
                if (count($user->products) >= $package->allowed_products) {
                    flash(__("Sorry! You have finished your upload limit.Please buy or renew package"))->success();
                    return redirect()->back();   
                }
            }
            ...
            flash(__("You don't have any subscription"))->success();
             return redirect()->back(); 
        }
    }
    ...
    public function seller_product_list(Request $request)
    {
        ...
        if (Auth::user()->products->count() > 0) {
            $package = Auth::user()->subscribes()->where('status',1)->orderBy('id','desc')->first();

            if (@$package->allowed_products > 0) {
                $allowed = count(Auth::user()->products);
            } else {
                $allowed = 0;
            }            
           
            if (\Carbon\Carbon::now()->format('Y-m-d') <= Auth::user()->seller->date && $allowed <= @$package->allowed_products){
                $data = Product::where('user_id', Auth::user()->id)->update(['published'=>1,'featured'=>1]);
            }
            else {
                $data = Product::where('user_id', Auth::user()->id)->update(['published'=>0,'featured'=>0]);
