        ...
        <div class="col-lg-4 col-sm-6">
            <div class="form-inline">
                <select class="form-control selectpicker form-control-sm"  data-minimum-results-for-search="Infinity" id="update_delivery_status">
                    ...
                    @endphp
                    @if (\Carbon\Carbon::now()->format('Y-m-d') <= Auth::user()->seller->date && $allowed <= $package->allowed_products && \App\Addon::where('unique_identifier', 'subscription')->first()->activated == 1 && Auth::user()->seller->delivery > 0)
                        <option value="delivery_request" @if (@$status == 'delivery_request') selected @endif>{{__('Delivery request')}}</option>
                    @endif
