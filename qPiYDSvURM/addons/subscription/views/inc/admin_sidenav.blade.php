            ...
            <div class="nano">
                <div class="nano-content">
                    ...
                    <ul id="mainnav-menu" class="list-group">
                        ...
                        @endif
                        @if (\App\Addon::where('unique_identifier', 'subscription')->first() != null && \App\Addon::where('unique_identifier', 'subscription')->first()->activated)
                        @php
                            $order_details = DB::table('order_details')
                                ->orderBy('id', 'desc')
                                ->where('seller_id', @\App\User::where('user_type', 'seller')->first()->id)
                                ->where('views', 0)
                                ->where('delivery_status', 'delivery_request')
                                ->select('orders.id')
                                ->distinct()
                                ->count();
                        @endphp
                        ...
                        <li class="{{ areActiveRoutes(['sales.index', 'sales.show'])}}">
                            <a class="nav-link" href="{{ route('sales.index') }}">
                                <i class="fa fa-money"></i>
                                <span class="menu-title">{{__('Total sales')}}
                                    @if (\App\Addon::where('unique_identifier', 'subscription')->first() != null && \App\Addon::where('unique_identifier', 'subscription')->first()->activated)                                        
                                        @if(@$order_details > 0)<span class="pull-right badge badge-info">{{ @$order_details }}</span>
                                        @endif
                                    @endif
                                </span>
                            </a>
                        ...
                        <li class="{{ areActiveRoutes(['subscription-index'])}}">
                            <a href="#">
                            ...
                            <!--Submenu-->
                            <ul class="collapse">
                                <li class="{{ areActiveRoutes(['seller_subscription'])}}">
                                    <a class="nav-link" href="{{ route('seller_subscription') }}">{{__('Seller Subscription Plans')}}</a>
                                </li>
                                <li class="{{ areActiveRoutes(['subscription-index'])}}">
                                    <a class="nav-link" href="{{ route('subscription-index') }}">{{__('Seller Subscriptions')}}</a>
                                </li>
                            </ul>
                        </li>
                        @endif
